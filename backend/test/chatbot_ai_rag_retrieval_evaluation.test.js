...
const evaluationCases = JSON.parse(readFileSync(
...
test('RAG-0510 fixed 50-case chat dataset memenuhi gate retrieval Fase 5', async (t) => {
  ...
  try {
    ...
    const calibrationCases = evaluationCases
      ...
      .map((item) => ({
        expected_relevant: item.expected_source_ids.length > 0,
    ...
    for (const item of evaluationCases) {
      ...
      const topThreeSourceIds = result.results.slice(0, 3).map((candidate) => candidate.source_id);
      const expectedRelevant = item.expected_source_ids.length > 0;
      const passed = expectedRelevant
        ? item.expected_source_ids.every((sourceId) => topThreeSourceIds.includes(sourceId))
        : result.selected_count === 0;
      outcomes.push({ item, result, passed, topThreeSourceIds });
    ...
    const topThreeAccuracy = passedCount / outcomes.length;
    const falsePositiveCount = outcomes.filter(({ item, result }) =>
      item.expected_source_ids.length === 0 && result.selected_count > 0).length;
    assert.ok(topThreeAccuracy >= 0.9, `top-3 accuracy hanya ${topThreeAccuracy}`);
    ...
    const categorySummary = Object.fromEntries(
      [...new Set(outcomes.map(({ item }) => item.category))].map((category) => {
        const categoryOutcomes = outcomes.filter(({ item }) => item.category === category);
        return [category, `${categoryOutcomes.filter(({ passed }) => passed).length}/${categoryOutcomes.length}`];
      })
    );
    t.diagnostic(`calibrated_threshold=${calibration.threshold.toFixed(6)}`);
    t.diagnostic(`top3_accuracy=${(topThreeAccuracy * 100).toFixed(2)}%`);
    t.diagnostic(`false_positive=${falsePositiveCount}`);
    t.diagnostic(`category_results=${JSON.stringify(categorySummary)}`);
  } finally {
    await close(db);
  }
});
