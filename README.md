...
Chatbot AI Token Optimization / Hybrid RAG berstatus **IN_PROGRESS — LOCAL**: migrations 024–026, default/maksimum output 2.048, prompt/usage/budget/retry, schema RAG + FTS, extractor/chunker, durable enqueue, gated polling, state job eksplisit, bounded index retry, stale-revision guard, publikasi lexical atomik, trigger KB manual, flow lifecycle triggers, serta manual reindex endpoint dengan ownership check (RAG-0311) telah diimplementasikan lokal. Endpoint `POST /api/chatbot-ai/rag/:sessionId/reindex` memvalidasi kepemilikan multi-tenant, mereset state job gagal/ready ke pending, auto-sync setting legacy, dan menjaga paritas OpenAPI 116/116. Empat puluh lima dari 128 item DONE; RAG-0011 tetap parsial. Focused reindex 8/8, focused RAG 80/80, dan full backend 227/227 lulus pada 2026-09-11. Poller dan budget moneter default nonaktif; database runtime belum dimigrasi, claim/processor/retrieval belum dibuat, dan deploy/UAT belum dilakukan.
