...
Chatbot AI Token Optimization / Hybrid RAG berstatus **IN_PROGRESS — LOCAL**: migrations 024/025, default/maksimum output 2.048, prompt/usage/budget/retry, schema RAG tenant-scoped + FTS, extractor/chunker, RAG-0301 durable enqueue, dan RAG-0302 gated polling worker telah diimplementasikan lokal. Poller hanya tersedia pada role `all`/`worker`, default nonaktif, memilih job due dari source aktif/current, membatasi batch, mencegah cycle overlap, dan tampil pada readiness. Tiga puluh enam dari 128 item DONE; RAG-0011 tetap parsial. Focused RAG 48/48 dan backend 175/175 lulus pada 2026-09-11. Eksperimen development tetap memakai snapshot ignored; file development tidak menjadi dependency runtime/staging/production. Budget moneter dan poller RAG default nonaktif; database runtime belum dimigrasi, claim/process/retrieval belum dibuat, dan deploy/UAT belum dilakukan.
