...
Chatbot AI Token Optimization / Hybrid RAG berstatus **IN_PROGRESS — LOCAL**: migrations 024–026, default/maksimum output 2.048, prompt/usage/budget/retry, schema RAG + FTS, extractor/chunker, durable enqueue, gated polling, state job eksplisit, bounded index retry, stale-revision guard, publikasi lexical atomik, trigger KB manual, serta flow lifecycle triggers (RAG-0308–0310) telah diimplementasikan lokal. `syncFlowKnowledgeSource` dan `deleteFlowKnowledgeSource` menyinkronkan sumber flow, mapping sesi, antrean job revision baru saat flow dibuat/diubah/import/status berubah, membersihkan sumber saat flow dihapus, dan memperbarui mapping saja tanpa job baru saat assignment sesi berubah. Empat puluh empat dari 128 item DONE; RAG-0011 tetap parsial. Focused RAG 92/92 dan backend 219/219 lulus pada 2026-09-11. Poller dan budget moneter default nonaktif; database runtime belum dimigrasi, claim/processor/reindex/retrieval belum dibuat, dan deploy/UAT belum dilakukan.
