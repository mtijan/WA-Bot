...
| Chatbot Flow large-data UX | Import/body limit is 60 MB, import/export progress is visible, list responses avoid large `nodes`, settings saves avoid resending `nodes`, and delivery counters are split into trigger/sent/failed. | `backend/src/config.js`, `backend/src/controllers/chatbot.controller.js`, `backend/src/services/whatsapp.service.js`, `frontend/src/components/ChatbotFlows.jsx` |
