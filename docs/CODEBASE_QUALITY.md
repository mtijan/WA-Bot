...
| Uploaded media tenant ownership | Uploaded media records are stored in `uploaded_media`; downloads are authenticated and limited to owner/admin. Legacy files without metadata fail closed. | `backend/src/services/upload.service.js`, `backend/src/controllers/upload.controller.js`, `backend/test/upload_tenant.test.js` |
...
| 2026-06-28 | SaaS entitlement/audit/media hardening | Added `subscription_plans`, entitlement middleware, Plans UI, `audit_logs`, uploaded media metadata/owner checks, race-safe device quota reservation, and OpenAPI parity 112/112. Backend test suite passes 92/92. |
